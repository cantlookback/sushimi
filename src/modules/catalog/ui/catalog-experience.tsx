...
import { ChevronRight, Minus, Plus, ShoppingBag, Sparkles } from "lucide-react";
      ...
      <section id="menu" className="mx-auto max-w-6xl px-4 md:px-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {products.map((product) => (
            <article key={product.id} className="group flex flex-col overflow-hidden rounded-3xl border border-[#e7e0d4] bg-[#fffdf8] shadow-[0_12px_35px_rgba(44,38,28,0.05)] transition-all duration-300 hover:-translate-y-1 hover:border-[#d8cdbd] hover:shadow-[0_18px_45px_rgba(44,38,28,0.11)]">
              ...
              <div className="flex flex-1 flex-col p-5">
                ...
                <div className="mt-5 flex items-center justify-between">
                  ...
                  {cart[product.id] ? <div className="flex items-center gap-3 rounded-full bg-[#1d1c19] p-1 text-white shadow-sm"><button aria-label="Уменьшить" onClick={() => changeQuantity(product, -1)} className="rounded-full p-2 transition-all duration-150 hover:bg-white/15 active:scale-90"><Minus size={16} /></button><span className="min-w-4 text-center text-sm font-extrabold">{cart[product.id]}</span><button aria-label="Увеличить" onClick={() => changeQuantity(product, 1)} className="rounded-full p-2 transition-all duration-150 hover:bg-white/15 active:scale-90"><Plus size={16} /></button></div> : <button disabled={!settings.ordering.acceptingOrders} onClick={() => changeQuantity(product, 1)} className="rounded-full bg-[#df3c2f] px-5 py-2.5 text-sm font-extrabold text-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#ee4a3d] hover:shadow-md active:translate-y-0 active:scale-[0.96] disabled:translate-y-0 disabled:bg-zinc-300 disabled:text-zinc-500 disabled:shadow-none">В корзину</button>}
                </div>
              </div>
            </article>
          ))}
