...
type AdminProduct = { id: string; categoryId: string; name: string; description: string; price: number; weightGrams: number | null; badges: string[]; available: boolean; imageUrl: string };
...
type Draft = { categoryId: string; name: string; description: string; priceRubles: string; weightGrams: string; badges: string; available: boolean };
...
export function CatalogManager({ categories, initialProducts, ingredients, initialRecipes }: { categories: Category[]; initialProducts: AdminProduct[]; ingredients: Ingredient[]; initialRecipes: Array<{ productId: string; ingredientId: string; quantity: number }> }) {
  ...
  async function save(event: FormEvent) {
    ...
    const payload = { categoryId: draft.categoryId, name: draft.name, description: draft.description, price: Math.round(Number(draft.priceRubles.replace(",", ".")) * 100), weightGrams: draft.weightGrams ? Number(draft.weightGrams) : null, badges: draft.badges.split(",").map((item) => item.trim()).filter(Boolean), available: draft.available };
    try {
